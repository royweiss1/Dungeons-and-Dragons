...
public class TileFactory {
        ...
        public Enemy produceEnemy(char tile, Position position, LevelManager lm, CLI cli) { // gamemanger.getLevelManger()
             Enemy e = enemiesMap.get(tile).get();
             e.initialize(position,(s)->cli.print(ConsoleColors.RED_BACKGROUND,s), lm::onEnemyDeath, lm::onUnitMove);
             return e;
        }

        public Player producePlayer(int idx, Position position, LevelManager lm, CLI cli, GameManager gm, InputProvider ip){
            if(selected == null) {
                Player p = playersList.get(idx).get();
                p.initialize( (s) -> cli.print(ConsoleColors.BLUE_BACKGROUND, s),gm::playerDeath,ip);
                selected = p;
            }
            lm.setPlayer(selected);
            lm.setInitPlayerPosition(position);
            return selected;
